...
from . import parser_bjh
from .net import WX_REFERER, download_image, fetch_article
from .parser import (
    Meta,
    collect_images,
    extract_head_styles,
...
def process_article(
    ...
    # 根据来源分流到对应解析器
    if source == "baijiahao":
        meta = parser_bjh.parse_meta(soup)
        content = soup.select_one(parser_bjh.CONTENT_SELECTOR)
        if content is None:
            raise RuntimeError(
                f"content container '{parser_bjh.CONTENT_SELECTOR}' not found"
            )
        _collect = parser_bjh.collect_images
        _rewrite = parser_bjh.rewrite_content
        referer = None
        ...
        if content is None:
            raise RuntimeError(
                "content div #js_content not found — login wall or invalid URL?"
            )
        _collect = collect_images
        _rewrite = rewrite_content
        referer = WX_REFERER
    ...
    if image_urls:
        with ThreadPoolExecutor(max_workers=img_workers) as pool:
            futs = [
                pool.submit(download_image, u, img_dir, client, referer)
                for u in image_urls
            ]
